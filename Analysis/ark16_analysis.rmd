...
```{r}
ark.data %>% 
  filter(variable == "626383" | variable == "533") %>% 
  mutate(Compartment = factor(Compartment, levels = c("Bulk Soil", "Rhizosphere", "Endosphere"))) %>% 
  ggplot(., aes(Date, value, color = Cultivar)) +
  geom_point() +
  geom_smooth(se = F) +
  facet_grid(.~variable+Compartment) +
  scale_color_brewer(palette = "Accent") +
  theme_minimal() +
  labs(x = "", y = "Relative Abundance (per Mille)") +
  theme(text = element_text(size = 10), axis.text.x = element_text(angle = 30, hjust = 1))
```

I think the association would be stronger if not for that one CLXL745 sample in September.

### Methanotrophs
Methanotrophic eubacteria could be on the other side of this methane story. There are two families with methanotrophic capabilities, Methylocystaceae and Methylococcaceae. There's an outlier genus in Methylocystaceae that cannot perform methanotrophy, Pleomorphomonas. 
```{r}
...
```
There are a few methanotrophs that seems to become enriched in the endopshere over time (but not in the rhizosphere). This pattern makes sense as the endosphere should have the highest levels of oxygen, a conditions that a methanotrophic lifestyle requires. Let's see if any of the OTUs have an interaction with time to become differentially abundant between the cultivars.
```{r}
...
```
It looks like two of the methanotrophs change their abundance patterns over the season, but there is not significant differences between the cultivars.
